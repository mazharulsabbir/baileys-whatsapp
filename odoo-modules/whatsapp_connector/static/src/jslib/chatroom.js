...
config.historyBack=()=>{}
useSubEnv({config})}
useBus(this.env.chatBus,'updateChatroomAction',async({detail:chatroomTab})=>{if(this.props.chatroomTab===chatroomTab){await this.model.load()}})}},updateURL(){if(this.env.chatBus){}else{super.updateURL()}},async discard(){await super.discard()
...
this.render(true)}})}},async openRecord(record,mode){if(this.props?.chatroomTab&&this.props?.chatroomOpenRecord){await this.props?.chatroomOpenRecord(record,mode)}else{await super.openRecord(record,mode)}}}
...
if(this.env.chatBus){useBus(this.env.chatBus,'updateChatroomAction',async({detail:chatroomTab})=>{if(this.props.chatroomTab===chatroomTab){await this.model.load()}})}}},async chatroomSelect(){const[selected]=await this.getSelectedResIds()
...
return this.uploadFile(file)},async uploadFile(file){const tmpId=getNextId()
...
return uploadDoneDeferred},async unlink(attachment){const abort=abortByAttachmentId.get(attachment.id)
...
await rpc('/mail/attachment/delete',assignDefined({attachment_id:attachment.id},{access_token:attachment.accessToken}))},clear(){abortByAttachmentId.clear()
...
async load(){const result=await this.env.services.orm.call(this.env.chatModel,'build_dict',[[this.id]],{context:this.env.context,limit:22})
...
for(const msg of messages){if(this.messagesIds.has(msg.id)){const oldMsg=this.messages.find(item=>item.id===msg.id)
oldMsg.updateFromJson(msg)}else{newMessages.push(new MessageModel(this,msg))}}
...
async createMessage(options){let msg=new MessageModel(this,options)
...
if(this.messagesIds.has(msg.id)){msg=this.messages.find(msg2=>msg2.id===msg.id)}else{this.messagesIds.add(msg.id)
...
return out}
async buildExtraObj(){await super.buildExtraObj()
if(this.resModelObj){this.resModelObj.message=this}else{if(this.resModel){const result=await this.env.services.orm.call(this.resModel,'read_from_chatroom',[this.resId],{context:this.env.context})
this.resModelObj={}
...
this.resModelObj.src=`/web/image?model=${res_model}&id=${res_id}&field=${res_field}`}else{if(['audio','sticker'].includes(this.ttype)){this.resModelObj=attach
...
createAttachObject(attachmentData){const attachment=new Attachment()
...
return out}
_contextHook(context){if((['res.partner','crm.lead'].includes(this.props.viewModel)&&this.props.selectedConversation?.isOwnerFacebook()&&!this.props.selectedConversation?.isWabaExtern())||this.props.selectedConversation?.isPrivate||this.props.selectedConversation?.isGroup){if('default_mobile'in context){delete context.default_mobile}
...
async openRecord(record,mode){if(mode==='edit'){const action={type:'ir.actions.act_window',name:_t('Edit'),view_type:'form',view_mode:'form',res_model:this.env.chatModel,views:[[this.props.formViewId,'form']],target:'new',res_id:record.resId,context:{...this.env.context,only_edit:true},}
...
replaceSelectedConversation(){const index=this.state.conversations.findIndex(conv=>conv.id===this.state.selectedConversation?.id)
let conv=null
if(index>=0){conv=new ConversationModel(this,{})
...
canHaveThisConversation(conversation){let out=true
...
if(this.state.isChatFiltering){let chatsFiltered=this.state.chatFilterResponse.map(item=>{return{name:item.name,values:item.values.map(item2=>{return{name:item2.name,values:item2.values.filter(this.evaluateFilter.bind(this))}})}})
...
if(ttype==='text'&&text){this.env.chatBus.trigger('setInputText',text)}else{out=this.props.selectedConversation.createMessage(options)}}else{this.env.services.dialog.add(WarningDialog,{message:_t('You must select a conversation.')})}
...
async willStart(){const{orm}=this.env.services
...
outMessages.push(await this.props.selectedConversation.createMessage(options))
await this.postCreateMessage(outMessages[outMessages.length-1])
attachmentsSent.push(attachment)}}}finally{this.inputRef.el.disabled=false
